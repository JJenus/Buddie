...
use \stdClass;
defined('CONFIGPATH') || define('CONFIGPATH', WRITEPATH."settings/");  
...
class Config
{
    ...
    public function load($sUsername)
    {
        ...
        if (is_file(CONFIGPATH . 'default.json')) {
            $this->oDefaultSettings = @json_decode(file_get_contents(CONFIGPATH . 'default.json'));
        }

        //load bot settings and merge
        if (is_file(CONFIGPATH . strtolower($sUsername) . '.json')) {
            $this->oSettings = @json_decode(file_get_contents(CONFIGPATH . strtolower($sUsername) . '.json'));
        } else {
    ...
    public function writeConfig()
    {
        if (isset($this->sUsername)) {
            file_put_contents(CONFIGPATH . strtolower($this->sUsername) . '.json', json_encode($this->oSettings, JSON_PRETTY_PRINT));
        }
    }
